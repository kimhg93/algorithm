...
public class Lessons17680 {
    ...
    public static void main(String[] args) throws Exception {
        int[] arr = {0};
        String[] arr2 = {"Jeju", "Pangyo", "NewYork", "newyork"};
        int n = 2;
        String str = "";

        long s = System.currentTimeMillis();
        System.err.println( solution(n,arr2) );
        ...
        for(int i=0; i<cities.length; i++){
            if(list.contains(cities[i].toUpperCase())){
                for(int j=0; j<list.size(); j++){
                    if(list.get(j).equals(cities[i].toUpperCase())){
                        list.remove(j);
                ...
                if(list.size() == cacheSize && cacheSize > 0) list.remove(0);
                if(cacheSize > 0) list.add(cities[i].toUpperCase());
                answer += 5;
            }
        }
        return answer;
