...
public class Lessons132265 {
    ...
    public static int solution(int[] topping) {
        ...
        int[] rear = new int[10001];
        int frontCnt = 0;
        int rearCnt = 0;

        for(int value : topping){
            if(rear[value] == 0) rearCnt++;
            rear[value]++;
        }

        while(pos != topping.length){
            int num = topping[pos-1];
            if(front[num] == 0) frontCnt++;
            front[num]++;
            rear[num]--;
            if(rear[num] == 0) rearCnt--;
            if(frontCnt == rearCnt) cnt++;
            pos++;
        }
