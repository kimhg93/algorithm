...
public class Lessons17677 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.err.println( solution(str, str2) );
    ...
    public static int solution(String str1, String str2) {
        ...
        for(int i=0; i<list1.size(); i++){
            for(int j=0; j<list2.size(); j++){
                if(list1.get(i).equals(list2.get(j))){ // 교집합 제거
                    list2.remove(j);
                    cnt++;
                    break;
                }
        ...
        if(list1.size() == 0 && list2.size() == 0) answer = 1/1 *65536;
        else answer = (int) (((double)cnt/(list1.size()+list2.size()))*65536); // 교집합은 제거했으므로 두개 합치면 합집합
    ...
    public static List<String> toList(String str){
        ...
        for(int i=0; i<str.length()-1; i++){
            if(check(str.charAt(i)) && check(str.charAt(i+1))) list.add(str.substring(i, i+2));
        }
        return list;
    }

    public static boolean check(char c){
        if(c>=65 && c<=90) return true; // 대문자
        else if(c>=97 && c<=122) return true; // 소문자
        else return false;
    }
}
