...
public class Lessons67257 {
    ...
    public static long solution(String exp) {
        ...
        PriorityQueue<Long> result = new PriorityQueue(Comparator.reverseOrder());

        int cnt = 0;
        int idx = 1;
        while(true){
            String s = exp.substring(cnt, idx);
            if(idx == exp.length()){
                list.add(s);
                break;
            }
        ...
        opList.add(new int[]{1, 2, 3});
        opList.add(new int[]{1, 3, 2});
        opList.add(new int[]{2, 1, 3});
        opList.add(new int[]{2, 3, 1});
        opList.add(new int[]{3, 1, 2});
        opList.add(new int[]{3, 2, 1});
        ...
        for(int[] arr : opList){
            ...
            result.offer(Math.abs(Long.parseLong(temp.get(0))));
        }

        return result.peek();
    }

    public static void cal(List<String> list, int op){
        String operator = op == 1 ? "-" : op == 2 ? "+" : "*";
        while(true){
            if(list.indexOf(operator) == -1) return;
            ...
            list.remove(idx-1);
            list.remove(idx-1);
            list.add(idx-1, Long.toString(op == 1 ? a-b : op == 2 ? a+b : a*b));
        }
    }
}
...
즉, + > - > * 또는 - > * > + 등과 같이 연산자 우선순위를 정의할 수 있으나 +,* > - 또는 * > +,-처럼 2개 이상의 연산자가 동일한 순위를 가지도록
...
이루어진 올바른 중위표기법(연산의 두 대상 사이에 연산기호를 사용하는 방식)으로 표현된 연산식입니다. 잘못된 연산식은 입력으로 주어지지 않습니다.
