...
public class Lessons42888 {
    ...
    public static void main(String[] args) throws Exception {
        int[] arr = {0};
        String[] arr2 = {"Enter uid1234 Muzi", "Enter uid4567 Prodo","Leave uid1234","Enter uid1234 Prodo","Change uid4567 Ryan"};
        int n = 0;
        String str = "";
    ...
    public static String[] solution(String[] record) {
        ...
        for(int i=0; i<record.length; i++){
            ...
            if(arr[0].indexOf("Change") == -1) list.add(new String[]{arr[1], arr[0]});
            if(arr.length == 3) uid.put(arr[1], arr[2]);
        }

        for(int i=0; i<list.size(); i++){
            record[i] = uid.get(list.get(i)[0]) +
                    (list.get(i)[1].equals("Enter") ? "님이 들어왔습니다.": "님이 나갔습니다.");
        }
        return record;
    }
}
