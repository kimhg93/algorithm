...
public class Lessons131704 {
    ...
    public static int solution(int[] order) {
        ...
        while(true){
            ...
            int num = order[idx];
            boolean flag = false;
            if(!st1.isEmpty() && st1.peek() == num){
                st1.pop();
                idx++;
            } else if(!st2.isEmpty() && st2.peek() == num){
                st2.pop();
                idx++;
            } else {
                while(true){
                    if(st1.isEmpty()) break;
                    flag = st1.peek() == order[idx];
                    if(!flag && !st2.isEmpty()) flag = (st2.peek() == num);
                    if(flag) break;
                    st2.push(st1.pop());
                }
            }
            if((idx == order.length || bfIdx == idx) && !flag) break;
        }
        return idx;
    }
...
하지만 컨테이너 벨트에 놓인 순서대로 택배상자를 내려 바로 트럭에 싣게 되면 택배 기사님이 배달하는 순서와 택배상자가 실려 있는 순서가 맞지 않아
배달에 차질이 생깁니다. 따라서 택배 기사님이 미리 알려준 순서에 맞게 영재가 택배상자를 실어야 합니다.
