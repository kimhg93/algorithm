...
public class Lessons17683 {
    ...
    public static void main(String[] args) throws Exception {
        String[] arr = {"03:00,03:30,FOO,CC#B", "04:00,04:08,BAR,CC#BCC#BCC#B"};
        int n = 0;
        String str = "CC#BCC#BCC#BCC#B";

        long s = System.currentTimeMillis();
        System.err.println(solution(str, arr));
    ...
    public static String solution(String m, String[] musicinfos) {
        String answer = "";
        int ms = 0;
        List<String> mList = mergeSharp(m);

        String result = "";
        int max = -1;

        for(int i=0; i<musicinfos.length; i++){
            String[] ar = musicinfos[i].split(",");
            String[] start = ar[0].split(":");
            String[] end = ar[1].split(":");
            int minutes = (Integer.parseInt(end[0]) - Integer.parseInt(start[0])) * 60 +
            ...
            for(int j=0; j<minutes; j++){
                if(idx < str.length()-1 && str.charAt(idx+1) == '#'){
                    sb.append(str.charAt(idx)).append(str.charAt(idx+1));
                    idx++;
                } else sb.append(str.charAt(idx));
                idx++;
                if(idx == str.length()) idx = 0;
            }

            List<String> list = mergeSharp(sb.toString());

            if(list.size() >= mList.size()){
                for(int j=0; j<=list.size()-mList.size(); j++){
                    if(mList.equals(list.subList(j, j+mList.size()))){
                        if(max < list.size()){
                            max = list.size();
                            result = ar[2];
                        }
    ...
    public static List<String> mergeSharp(String str){
        ...
        for(int i=0; i<str.length(); i++){
            if(i < str.length()-1 && str.charAt(i+1) == '#'){
                ...
                i++;
            } else list.add(String.valueOf(str.charAt(i)));
        }
