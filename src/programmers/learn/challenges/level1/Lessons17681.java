...
public class Lessons17681 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.err.println( solution(n, arr, arr2) );
    ...
    public static String[] solution(int n, int[] arr1, int[] arr2) {
        ...
        for(int i=0; i<arr1.length; i++){
            sArr1[i] = fill(Integer.toString(arr1[i], 2), n);
            sArr2[i] = fill(Integer.toString(arr2[i], 2), n);
        }

        for(int i=0; i<sArr1.length; i++){
            StringBuilder sb = new StringBuilder();
            for(int j=0; j<sArr1[i].length(); j++){
                int a = Integer.parseInt(sArr1[i].substring(j, j+1));
                int b = Integer.parseInt(sArr2[i].substring(j, j+1));
                if(a+b > 0) sb.append("#");
                else sb.append(" ");
            }
