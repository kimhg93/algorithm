...
fun main() {
    ...
    val find: MutableList<Int> = ArrayList()

    while(st.hasMoreTokens()){
        find.add(st.nextToken().toInt())
    }

    val ldq: Deque<Int> = LinkedList()
    val rdq: Deque<Int> = LinkedList()
    ...
    while(idx < m){
        ...
        while(true){
            if(ldq.peekFirst() == find[idx]){
                ldq.pollFirst()
                break
            }
            ldq.addLast(ldq.pollFirst())
            lcnt++
        }

        while(true){
            if(rdq.peekFirst() == find[idx]){
                rdq.pollFirst()
                break
            }
            rdq.addFirst(rdq.pollLast())
            rcnt++
        }

        result += min(lcnt, rcnt)
        idx++
    }
    println(result)
}
...
2 초	128 MB	42158	25129	20549	61.169%
문제
