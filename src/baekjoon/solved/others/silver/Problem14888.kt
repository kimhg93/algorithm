...
fun main() {
    ...
    val op: MutableList<Int> = ArrayList()
    var st = StringTokenizer(readLine())

    repeat(n){ nums.add(st.nextToken().toLong()) }

    st = StringTokenizer(readLine())
    repeat(4) {
        val cnt = st.nextToken().toInt()
        when(it) {
            0 -> repeat(cnt){ op.add(0) }
            1 -> repeat(cnt){ op.add(1) }
            2 -> repeat(cnt){ op.add(2) }
            3 -> repeat(cnt){ op.add(3) }
        }
    }

    permutation(op, IntArray(op.size), BooleanArray(op.size), 0)

    println(max)
    println(min)
}

fun permutation(arr: MutableList<Int>, result: IntArray, visit: BooleanArray, d: Int) {
    if (d == result.size) {
        var sum = 0L
        for(j in result.indices){
            val a: Long = if(j == 0) nums[j] else sum
            when(result[j]){
                0 -> sum = a + nums[j+1]
                1 -> sum = a - nums[j+1]
                2 -> sum = a * nums[j+1]
    ...
    for (i in arr.indices) {
        if (!visit[i]) {
            visit[i] = true
            result[d] = arr[i]
            permutation(arr, result, visit, d + 1)
            visit[i] = false
        }
    }
}
...
2 초	512 MB	91801	41818	27920	47.620%
문제
